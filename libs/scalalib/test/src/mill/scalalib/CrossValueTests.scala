...
object CrossValueTests extends TestSuite {
  ...
  def crossModuleTest(): Unit =
    UnitTester(modules, null).scoped { eval =>
      // eval the cross task itself - this should list its possible values
      val Right(Result(res, _)) =
        eval.apply(modules.theScalaVersion.unresolved(Map())): @unchecked
      assert(res == Seq("2.13.16", "3.3.4"))

      val Right(Result(sourcesRes, _)) =
        eval(modules.crossValueModule.sources.unresolved(Map())): @unchecked
      ...
      val Right(Result(compileCpRes, _)) =
        eval(modules.crossValueApp.compileClasspath.unresolved(Map())): @unchecked
      ...
      val Right(Result(legacy213CrossAppRes, _)) =
        eval(modules.legacyCrossApp("2.13.16").compileClasspath.unresolved(Map())): @unchecked
      val Right(Result(legacy3CrossAppRes, _)) =
        eval(modules.legacyCrossApp("3.3.4").compileClasspath.unresolved(Map())): @unchecked
      ...
      val Right(Result(crossValueAppRunCpRes, _)) =
        eval(modules.crossValueApp.runClasspath.unresolved(Map())): @unchecked
      ...
      val Right(Result(legacy213CrossAppRunCpRes, _)) =
        eval(modules.legacyCrossApp("2.13.16").runClasspath.unresolved(Map())): @unchecked
      ...
      val Right(Result(legacy3CrossAppRunCpRes, _)) =
        eval(modules.legacyCrossApp("3.3.4").runClasspath.unresolved(Map())): @unchecked
