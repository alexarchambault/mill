...
public class MillProcessLauncher {
  ...
  private static Path computeCacheFile(
      String name, Function<Path, Boolean> validate, int count, OutFolderMode outMode)
      throws IOException {
    try {
      ...
      Path cacheFile = Paths.get(".")
          .resolve(outFor(outMode))
          .resolve("mill-" + name + suffix)
          .toAbsolutePath();
      if (Files.exists(cacheFile))
      ...
      return computeCacheFile(name, validate, count + 1, outMode);
    }
  }
  ...
  static Path cachedLauncher(
      ...
      OutFolderMode outMode) {
    try {
      Path cacheFile = computeCacheFile(name, validate, 0, outMode);
